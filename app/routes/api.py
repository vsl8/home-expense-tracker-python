...
def update_expense_type(type_id):
    ...
    if data.get("name"):
        ...
        existing = ExpenseType.query.filter(ExpenseType.name == data["name"], ExpenseType.id != type_id).first()
        if existing:
            return jsonify({"error": "Expense type with this name already exists"}), 400
...
def get_dashboard_stats():
    ...
    # Today's expenses
    today_total = db.session.query(func.sum(Expense.amount)).filter(Expense.date == today).scalar() or 0
    ...
    return jsonify(
        {
            ...
            "category_breakdown": [
                {"name": stat[0], "color": stat[1], "total": round(stat[2], 2), "count": stat[3]}
