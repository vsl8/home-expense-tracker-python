...
def export_to_pdf():
    ...
    elements.append(Paragraph(f"Period: {start_date.isoformat()} to {end_date.isoformat()}", 
                              styles['Normal']))
...
def get_chart_data():
    ...
    if chart_type == 'pie':
        ...
        return jsonify({
            ...
            'data': [round(d[2], 2) for d in data]
        })
        ...
        data = db.session.query(
            ...
            Expense.date >= start_date,
            Expense.date <= end_date
        ).group_by(ExpenseType.id).order_by(func.sum(Expense.amount).desc()).all()
        ...
        return jsonify({
            ...
            'data': [round(d[1], 2) for d in data]
        })
